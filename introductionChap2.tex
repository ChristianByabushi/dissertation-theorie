...
In DRC, especially in the eastern party, users face similar issues. This chapter emphasizes methods or techniques that can be used to address this problem and relatively reduce spam. One of the prominent techniques suggested is based on Artificial Intelligence, particularly Machine Learning algorithms.
   ...
   \section{Tools and Techniques} 
	...
	\subsubsection{Back-end Development with Python (Django) and SQL :} 
	 Python is a powerful language appeared in 1980 firstly implemented by Guido van Rossum at \textit{CWI} in the Netherlands as a successor to the \textit{ABC} programming language \cite{tulchak2016history}. It newest version is \textit{Python 3} \footnote{Python : \url{https://www.python.org/}}  which is available for all most environments, either \textit{Macos} or \textit{Linux} and \textit{Windows}.
	 ...
	 For sure, the ORM mapps the relational \textit{SQL (Structured Query language)} which is the   
