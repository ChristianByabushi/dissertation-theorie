...
\subsection{\large Data Science tools and frameworks}
...
Pandas has two main structures: Series and DataFrame. The first is one-dimensional array, while the second is a two-dimensional table that is similar to a spreadsheet or SQL. Let's break in example: 
- Series:
\begin{lstlisting}[style=stylepython]
...
2  Charlie   35
\end{lstlisting} 
Assume we want to apply some characteristic on a column :
\begin{lstlisting}[style=stylepython]
...
\subsubsection{Data Visualization With \textit{Matplotlib}}
Matplotlib is a open-source, flexible and customizable Python package used for generating 2D plotting that generates production-quality graphs. In addition to this, it includes the capacity of saving images in different output formats (JPG, PNG, PS and others) \cite{tosi2009matplotlib}. 

To get started with it, we just go to the official page and download the version \footnote{Matplotlib: \url{https://matplotlib.org/}}. However, for those using some \textit{IDE} like \textit{Anaconda}, packages like : \textit{numpy}, \textit{pandas}, and \textit{matplotlib} are incorporated inside.
When working with data visualization, the type of plot to choose should depend on the nature the data. In the table , the overview of types of plots and when to use them is given. However, some plots can be combined even though they are primarily made for whether categorical or numerical visualization. 

\noindent The usage of \textit{matplotlib} properties require a good manipulation of data, often done by the \textit{numpy} and \textit{pandas}. Let's break into examples of it usage: 
First all, the import of \textit{matplotlib} is required for the manipulation like this :
\begin{lstlisting}[style=stylepython]
