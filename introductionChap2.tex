...
In DRC, especially in the eastern party, users face similar issues. This chapter emphasizes methods or techniques that can be used to address this problem and relatively reduce spam. One of the prominent techniques suggested is based on Artificial Intelligence, particularly Machine Learning algorithms.
   ...
   \section{Tools and Techniques} 
	...
	\subsubsection{Back-end Development with Python (Django) and SQL :}  
    ...
    % SQL Query
  \texttt{\textcolor{blue}{SELECT} \textcolor{green}{*} \textcolor{blue}{FROM} \textcolor{purple}{employees} \textcolor{blue}{WHERE} \textcolor{purple}{salary} \textcolor{blue}{>} \textcolor{red}{4000};} \newline While Django accomplishes the same task using the following code:
  
  % Django ORM Query 
  \texttt{\textcolor{blue}{from} myapp.models \textcolor{blue}{import} Employee}
  
  \texttt{employees = \textcolor{black}{Employee.objects.\textcolor{green}{filters}(salary\_\_gt = \textcolor{red}{4000})}}
   \\
   
   \textbf{{How have we gotten till there }{den}}? In fact, the project must be running for doing that. For installing \textit{Python}, just go to the official website for the  download, no matter the OS version, either \textit{ \href{https://www.python.org/downloads/macos/}{MACOS} , \href{https://www.python.org/downloads/windows/}{Windows} } or \href{https://www.python.org/downloads/source/}{\textit{Linux/UNIX}} since \textit{Python} is portable. Indeed, the last version at the writing of this project was Python 3.12.0. To test if the it is working, just enter the command \textcolor{blue}{python} as in figure \ref{fig:testpython}.
   \begin{figure}
   	\centering
   ...
   Next, for installing \textit{Django}, the steps remain pretty the same, going on the official page and following the guides as resumed in the as follows:   
  ...
  In\textit{ \textbf{MACOS/Linux}} : Only the way of activating the VM changes. Then just by entering the following command:\newline
  \texttt{source myenv/bin/activate}; the virtual will be functioning. 
  \newline
   \textbf{Finally}, as the \textit{V.M} is working Django, can be installed, by the command :  \texttt{pip install Django}. At the writing of this project, the last version 4.2.6. The bit steps to follow for installing are demonstrated in the figure \ref{fig:installdjango}
\begin{figure}
	\centering
...
\end{figure} 
 \newline However, the configuration about the app created, ought to be made lest it should raise the error. Thus, in settings file, present in the project folder the properties called \textit{INSTALLED\_APPS}, as in the figure \ref{fig:appinstallname}.\newline After, this we can just enter the commands : \texttt{ python manage.py makemigrations; 
 python manage.py migrate;}. \newline For sure, the first command says to \textit{Django} the new changes, and the second applies or assesses them. 
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{appInstallName}
	\caption{Adding the appname in INSTALLED\_APPS dependencies }
	\label{fig:appinstallname}
\end{figure} 
\\
\newline
\textbf{How does \textit{Django} deal with the database ?} 

\textit{Django} deals with databases through \textit{ORM} as mentioned above. Its management involves to follow different steps such as:
...
\textbf{What does \textit{Django} for interacting the APIs ?}
...
\subsubsection{Front-end Development with HTML, CSS and Js :} 
